...
def encode(phrase):
	...
	for letter in phrase:
		if letter in codebook.keys():
			replacement_letter = codebook[letter]
			cipher += replacement_letter
		else:
			cipher += letter
...
def sort_by_word_length(words):
	...
	word_dictionary = {len(word): [] for word in words}
...
def get_pirate_talk(phrase):
	...
	for word in phrase_list:
		if word in pirate_code_book.keys():
			replacement_word = pirate_code_book[word]
			pirate_phrase_list.append(replacement_word)
		else:
			pirate_phrase_list.append(word)
			
	pirate_phrase = " ".join(pirate_phrase_list)
...
def adv_get_top_letter(input_string):
	...
	letter_dictionary = {letter: 0 for letter in input_string if letter.isalpha()}
	...
	for key, value in letter_dictionary.items():
		lil_list = [key, value]
		if lil_list[1] > comparison_list[1]:
			most_common_letter = lil_list[0]
			comparison_list = lil_list
			
	most_common_letter_list = [most_common_letter]
	
	for key in letter_dictionary.keys():
		if key != most_common_letter and letter_dictionary[key] == letter_dictionary[most_common_letter]:
			most_common_letter_list.append(key)
...
def adv_alpha_sort_by_word_length(words):
	...
	for key, value in word_dictionary.items():
		word_dictionary[key] = sorted(value)
